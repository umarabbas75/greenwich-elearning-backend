/**
 * _audit-user-version-deep.ts  (STRICTLY READ-ONLY)
 *
 * Deep diagnostic for "why is this user pinned to version X and not Y".
 * Replicates the read-side resolution logic WITHOUT the side effects that the
 * live service methods have (zero-progress bump, drift auto-publish) so it is
 * safe to run against production.
 *
 *   yarn ts-node -r tsconfig-paths/register scripts/_audit-user-version-deep.ts <email> [courseId]
 *
 * Defaults to the NEBOSH IGC course id.
 */
...
async function main() {
  ...
  const userCaseStudyProgress = [...distinctSectionIds].filter((id) =>
    caseStudyIds.has(id),
  ).length;
  console.log('\n── Case Study sections ──');
  console.log({
  ...
  // ── Denominator comparison: what % would each version yield? ──
  ...
  if (uc?.enrolledVersionId) {
    ...
    if (orphaned.length > 0) {
      console.log(
        '  ⚠ User has progress on sections that are NOT in their pinned version snapshot:',
      );
  ...
  if (uc?.enrolledVersionId && latest && uc.enrolledVersionId !== latest.id) {
    console.log(
      `  ⚠ User is pinned to v${uc.enrolledVersion?.versionNumber} but latest is v${latest.versionNumber}.`,
    );
    console.log(
      '    syncSectionToLatestVersion / syncQuizToLatestVersion etc. only update the LATEST version,',
    );
    console.log(
      '    so any admin CONTENT edits will NOT reach this learner. This is "profile not synced".',
    );
