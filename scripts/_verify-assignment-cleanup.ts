/**
 * One-off verification: confirm that assignment deletions left no orphans.
 *
 *   - assignment_submissions whose assignmentId no longer exists
 *   - notifications whose groupKey points at a deleted assignmentId
 *     (assignment-created:* / assignment-submitted:*)
 *   - ASSIGNMENT_GRADED notifications whose referenceId is a deleted submission
 *
 * Read-only.
 */
...
async function main() {
  ...
  for (const a of assignments) {
    console.log(
      `  - ${a.id}  · "${a.title}"  · submissions=${a._count.submissions}  · createdAt=${a.createdAt.toISOString()}`,
    );
  }
  console.log();

  // 1. Orphan submissions (FK should make this impossible, but verify).
  ...
  console.log(
    `Submissions total: ${allSubmissions.length}  · orphans: ${orphanSubs.length}`,
  );
  if (orphanSubs.length) {
    for (const s of orphanSubs.slice(0, 10)) {
      console.log(`  ! orphan submission ${s.id} → assignment ${s.assignmentId}`);
    }
    if (orphanSubs.length > 10) console.log(`  …and ${orphanSubs.length - 10} more`);
  }
  console.log();

  // 2. Orphan bell notifications keyed by assignmentId in groupKey.
  ...
  console.log(
    `Lifecycle notifications (created/submitted): ${groupKeyNotifications.length}  · orphans: ${orphanGroupKey.length}`,
  );
  if (orphanGroupKey.length) {
    for (const n of orphanGroupKey.slice(0, 10)) {
      console.log(
        `  ! orphan ${n.type}  groupKey=${n.groupKey}  user=${n.userId}  at ${n.createdAt.toISOString()}`,
      );
    }
  ...
  // 3. ASSIGNMENT_GRADED notifications whose referenceId is a deleted submission.
  ...
  const orphanGraded = gradedNotifications.filter(
    (n) => n.referenceId && !liveSubIds.has(n.referenceId),
  );
  console.log(
    `ASSIGNMENT_GRADED notifications: ${gradedNotifications.length}  · orphans: ${orphanGraded.length}`,
  );
  if (orphanGraded.length) {
    for (const n of orphanGraded.slice(0, 10)) {
      console.log(
        `  ! orphan graded  refSubmissionId=${n.referenceId}  user=${n.userId}  at ${n.createdAt.toISOString()}`,
      );
    }
