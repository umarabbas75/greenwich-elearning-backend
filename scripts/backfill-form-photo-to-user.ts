/**
 * backfill-form-photo-to-user.ts
 *
 * Copies `metadata.userPhoto` from completed course forms onto `users.photo`
 * when the user has no global profile photo yet.
 *
 *   yarn ts-node -r tsconfig-paths/register scripts/backfill-form-photo-to-user.ts --dry-run
 *   yarn ts-node -r tsconfig-paths/register scripts/backfill-form-photo-to-user.ts
 *   yarn ts-node -r tsconfig-paths/register scripts/backfill-form-photo-to-user.ts --email=saiff9642@gmail.com
 */
...
import {
  ...
  userHasGlobalPhoto,
} from '../src/utils/promote-form-photo-to-user';
...
async function main() {
  ...
  const usersWithoutPhoto = users.filter((u) => !userHasGlobalPhoto(u.photo));
  ...
  for (const row of completions) {
    ...
    const photoUrl = extractUserPhotoFromMetadata(row.metadata);
    if (!photoUrl) {
      continue;
    }
  ...
  for (const c of candidates) {
    console.log(
      `  ${c.email} | form ${c.formId} | ${c.photoUrl.slice(0, 60)}...`,
    );
  }
