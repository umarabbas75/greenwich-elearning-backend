/**
 * prune-orphan-course-versions.ts
 *
 * One-off cleanup for course version snapshot bloat.
 *
 * A version is "orphan" when it has ZERO enrollments pinned to it
 * (user_courses.enrolledVersionId). Learners on this version get their
 * curriculum from the live tree via floating enrollment, so removing an
 * orphan snapshot never affects a learner.
 *
 * Postgres refuses to delete a version referenced by an enrollment
 * (enrolledVersion has onDelete: Restrict), so we have a hard safety net.
 *
 * Retention policy applied per course:
 *   - Keep every version with enrollments > 0
 *   - Keep the current latest version (even if orphan) so admins see history
 *   - Delete every other orphan
 *   - If the current latest is deleted, promote the highest-remaining
 *     version to isLatest = true
 *
 * Usage:
 *   yarn ts-node -r tsconfig-paths/register scripts/prune-orphan-course-versions.ts --dry-run
 *   yarn ts-node -r tsconfig-paths/register scripts/prune-orphan-course-versions.ts
 *   yarn ts-node -r tsconfig-paths/register scripts/prune-orphan-course-versions.ts --course-id=<uuid>
 *   yarn ts-node -r tsconfig-paths/register scripts/prune-orphan-course-versions.ts --keep-latest-only  (also delete current latest if orphan)
 *
 * After a real run consider `VACUUM (FULL, ANALYZE)` in the Neon SQL editor.
 */
...
async function loadVersionRow(id: string): Promise<VersionRow> {
...
async function buildPlan(): Promise<CoursePlan[]> {
  ...
  for (const course of courses) {
    ...
    const versions = await Promise.all(
      versionIds.map((v) => loadVersionRow(v.id)),
    );

    // Sort ascending so promotion picks the highest-remaining as latest.
    versions.sort((a, b) => a.versionNumber - b.versionNumber);

    const keep: VersionRow[] = [];
    const drop: VersionRow[] = [];
    ...
    for (const v of versions) {
      ...
      // Orphan version. Keep current latest by default so admins retain
      // history; --keep-latest-only overrides that.
    ...
    // If the current latest is being dropped, we must promote another.
    let promoteLatestTo: VersionRow | undefined;
...
function describeVersion(v: VersionRow): string {
  const flags: string[] = [];
  if (v.isLatest) flags.push('latest');
  if (v.enrollments > 0) flags.push(`${v.enrollments} enrolled`);
...
async function printPlan(plans: CoursePlan[]) {
  ...
  for (const plan of plans) {
    ...
    if (plan.promoteLatestTo) {
      console.log(
        `  → Promote v${plan.promoteLatestTo.versionNumber} to isLatest = true`,
      );
    }
  }
  ...
  console.log(
    `  Est. content payload freed: ${formatBytes(totalDropBytes)} (content only, excluding indexes/TOAST overhead)`,
  );
}

async function applyPlan(plans: CoursePlan[]) {
  for (const plan of plans) {
    ...
    await prisma.$transaction(
      async (tx) => {
        if (plan.promoteLatestTo) {
          ...
          console.log(
            `  Promoted v${plan.promoteLatestTo.versionNumber} → isLatest`,
          );
        }

        for (const v of plan.drop) {
          // Cascade deletes handle course_version_modules/chapters/sections/quizzes.
          // enrolledVersion FK is Restrict, so this throws if any enrollment
          // still references the version (defence in depth).
...
async function main() {
  ...
  if (courseIdFilter) console.log(`Course filter: ${courseIdFilter}`);
  if (keepLatestOnly)
  ...
  const dbSizeAfter = await prisma.$queryRaw<{ size: string }[]>`
    SELECT pg_size_pretty(pg_database_size(current_database())) AS size
  `;
