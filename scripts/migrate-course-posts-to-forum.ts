/**
 * migrate-course-posts-to-forum.ts
 *
 * Copies in-course Discussions (`posts` / `comments`) into Community Study
 * threads. Idempotent via `forum_threads.sourcePostId`.
 *
 *   yarn ts-node -r tsconfig-paths/register scripts/migrate-course-posts-to-forum.ts
 *   yarn ts-node -r tsconfig-paths/register scripts/migrate-course-posts-to-forum.ts --apply
 */

import * as dotenv from 'dotenv';
import { ForumCourseScope, PrismaClient } from '@prisma/client';
...
async function studyCategory() {
  const required = await prisma.forumCategory.findMany({
    where: { courseScope: ForumCourseScope.REQUIRED, isActive: true },
...
async function main() {
  ...
  if (!category) {
    console.error(
      'No active category with courseScope=REQUIRED. Wave 1 Study board is missing.',
    );
    process.exit(1);
  }
  ...
  const migrated = new Set(
    already.map((row) => row.sourcePostId).filter((id): id is string => !!id),
  );
  ...
  for (const post of posts) {
    ...
    const lastActivityAt =
      post.comments[post.comments.length - 1]?.createdAt ?? post.updatedAt;

    console.log(
      `${apply ? 'CREATE' : 'would create'} "${post.title}" ` +
