/**
 * backfill-form-address-to-user.ts
 *
 * Copies registration-form address fields from completed course forms onto
 * `users.address` when the user has no global address yet.
 *
 *   yarn ts-node -r tsconfig-paths/register scripts/backfill-form-address-to-user.ts --dry-run
 *   yarn ts-node -r tsconfig-paths/register scripts/backfill-form-address-to-user.ts
 *   yarn ts-node -r tsconfig-paths/register scripts/backfill-form-address-to-user.ts --email=saiff9642@gmail.com
 */
...
} from '../src/utils/promote-form-address-to-user';
...
async function main() {
  ...
  const usersWithoutAddress = users.filter(
    (u) => !userHasGlobalAddress(u.address),
  );
  ...
  const bestByUser = new Map<string, Candidate>();
  ...
  for (const row of completions) {
    ...
    const address = extractUserAddressFromMetadata(row.metadata);
    if (!address) {
      continue;
    }
  ...
  const candidates = [...bestByUser.values()].sort((a, b) =>
  ...
  for (const c of candidates) {
    console.log(
      `  ${c.email} | form ${c.formId} | ${c.address.slice(0, 80)}${
        c.address.length > 80 ? '...' : ''
      }`,
    );
  }
