...
async function main() {
  ...
  `;
    ...
    console.log(`Found ${rows.length} user-course completion row(s) showing < 100% live progress.\n`);
    ...
    for (const [, list] of byCourse) {
        ...
        console.log(`▸ "${title}"  (${total} sections, ${list.length} affected user(s))`);
        for (const r of list) {
            const name = [r.first_name, r.last_name].filter(Boolean).join(' ').trim() || '—';
            const pct = Number(r.live_percentage).toFixed(2);
            const completedAt = r.course_completed_at.toISOString().split('T')[0];
            console.log(`    ${r.email.padEnd(36)} · ${name.padEnd(24)} · completed ${completedAt} · ${r.completed_sections}/${r.total_sections} = ${pct}%`);
        }
        console.log();
    }
    console.log(`Total: ${rows.length} affected completer(s) across ${byCourse.size} course(s).\n`);
    console.log('These users will jump back to 100% the moment the freeze-at-100 patch is deployed — no data backfill needed.\n');
}
main()
    .catch((err) => {
...
//# sourceMappingURL=_audit-completion-mismatch.js.map