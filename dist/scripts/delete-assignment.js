...
async function resolveAssignment() {
    ...
    if (matches.length === 0) {
        ...
        const available = fuzzy
            ...
            .join('\n');
        throw new Error(`No assignment matched title "${title}" (case-insensitive).\n\nAvailable assignments:\n${available}`);
    }
    if (matches.length > 1) {
        const lines = matches
            .map((a) => `  • ${a.title} [${a.id}] — course: ${a.course.title}, submissions: ${a.submissions.length}`)
            .join('\n');
        throw new Error(`Multiple assignments matched title "${title}". Pass --id= explicitly.\n\nMatches:\n${lines}`);
    }
    return matches[0];
}
async function main() {
    ...
    if (assignment.submissions.length > 0) {
        ...
        for (const s of assignment.submissions) {
            ...
            console.log(`  • ${who} — ${s.status} — submitted ${s.submittedAt.toISOString().slice(0, 10)}`);
        }
        console.log();
    }
...
//# sourceMappingURL=delete-assignment.js.map