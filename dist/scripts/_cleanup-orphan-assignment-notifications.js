...
async function main() {
    ...
    const orphanGraded = gradedRows.filter((n) => n.referenceId && !liveSubmissionIds.has(n.referenceId));
    const orphanIds = [
        ...orphanGroupKey.map((n) => n.id),
        ...orphanGraded.map((n) => n.id),
    ];
    console.log(`Lifecycle orphans (created/submitted by groupKey): ${orphanGroupKey.length}`);
    console.log(`Graded orphans (referenceId → missing submission): ${orphanGraded.length}`);
...
//# sourceMappingURL=_cleanup-orphan-assignment-notifications.js.map