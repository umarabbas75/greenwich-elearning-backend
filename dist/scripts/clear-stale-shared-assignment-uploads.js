...
const STALE_FILE_URL = 'https://res.cloudinary.com/dp9urvlsz/raw/upload/v1781429861/my_uploads/GIC1_IG1-0061-ENG-OBE-Answer-sheet-V1.pdf';
...
async function main() {
    ...
    const keep = matches.filter((s) => s.student.email.toLowerCase() === KEEP_EMAIL.toLowerCase());
    const toClear = matches.filter((s) => s.student.email.toLowerCase() !== KEEP_EMAIL.toLowerCase());
    console.log('Stale Cloudinary URL:');
    console.log(' ', STALE_FILE_URL);
    console.log('\nKEEP (Imad — do not delete):', keep.length);
    for (const s of keep) {
        console.log(`  ${s.student.email} | ${s.status} score=${s.score ?? 'n/a'} | ${s.assignment.title}`);
        console.log(`    submissionId=${s.id}`);
    }
    console.log('\nCLEAR (bad shared-URL submissions):', toClear.length);
    for (const s of toClear) {
        console.log(`  ${s.student.firstName} ${s.student.lastName} <${s.student.email}>`);
        console.log(`    submissionId=${s.id}`);
        console.log(`    assignment=${s.assignment.title}`);
        console.log(`    status=${s.status} score=${s.score ?? 'n/a'} gradedAt=${s.gradedAt?.toISOString() ?? 'n/a'}`);
...
function printEmailTemplate(rows) {
    ...
    for (const s of rows) {
        ...
        console.log(`Subject: Action required — please re-submit your NEBOSH mock assessment\n`);
...
//# sourceMappingURL=clear-stale-shared-assignment-uploads.js.map