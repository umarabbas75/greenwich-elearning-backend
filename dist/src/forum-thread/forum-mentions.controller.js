"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ForumMentionsController = void 0;
const common_1 = require("@nestjs/common");
const passport_1 = require("@nestjs/passport");
const decorator_1 = require("../decorator");
const forum_thread_service_1 = require("./forum-thread.service");
let ForumMentionsController = class ForumMentionsController {
    constructor(threads) {
        this.threads = threads;
    }
    search(user, q, threadId, courseId) {
        return this.threads.searchMentions(user, { q, threadId, courseId });
    }
};
exports.ForumMentionsController = ForumMentionsController;
__decorate([
    (0, common_1.UseGuards)((0, passport_1.AuthGuard)('cJwt')),
    (0, common_1.Get)('/mentions'),
    __param(0, (0, decorator_1.GetUser)()),
    __param(1, (0, common_1.Query)('q')),
    __param(2, (0, common_1.Query)('threadId')),
    __param(3, (0, common_1.Query)('courseId')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, String, String, String]),
    __metadata("design:returntype", void 0)
], ForumMentionsController.prototype, "search", null);
exports.ForumMentionsController = ForumMentionsController = __decorate([
    (0, common_1.Controller)('forum'),
    __metadata("design:paramtypes", [forum_thread_service_1.ForumThreadService])
], ForumMentionsController);
//# sourceMappingURL=forum-mentions.controller.js.map