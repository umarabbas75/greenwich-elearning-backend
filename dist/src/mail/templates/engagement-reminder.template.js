          ...
          <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:8px;padding:32px;max-width:560px;">
            <tr><td style="font-size:15px;line-height:1.6;color:#1f2933;">
              ...
              <div style="margin:28px 0;">
                <a href="${safeUrl}" style="display:inline-block;background:#0b6b3a;color:#ffffff;text-decoration:none;padding:12px 24px;border-radius:6px;font-size:15px;">${escapeHtml(ctaLabel)}</a>
              </div>
...
function renderEngagementReminder(mail) {
    ...
    if (mail.reminderType === mail_types_1.ReminderType.NEVER_STARTED) {
        ...
        const html = layout(`<p style="margin:0 0 16px;">Dear ${firstName},</p>
       <p style="margin:0 0 16px;">Our records show that you enrolled in <strong>${courseTitle}</strong> but have not yet begun the course. To make progress towards your certification, we encourage you to begin when convenient.</p>`, 'Begin course', mail.courseUrl);
        const text = `Dear ${mail.firstName || 'there'},

Our records show that you enrolled in ${mail.courseTitle} but have not yet begun the course. To make progress towards your certification, we encourage you to begin when convenient.
...
${SIGN_OFF}`;
    ...
    const html = layout(`<p style="margin:0 0 16px;">Dear ${firstName},</p>
     <p style="margin:0 0 16px;">We noticed it has been some time since your last activity in <strong>${courseTitle}</strong>. Your progress has been saved, and you may resume from where you left off at any time.</p>`, 'Resume course', mail.courseUrl);
    const text = `Dear ${mail.firstName || 'there'},

We noticed it has been some time since your last activity in ${mail.courseTitle}. Your progress has been saved, and you may resume from where you left off at any time.
