    ...
    switch (mail.kind) {
        ...
        <p style="margin-top:8px;padding:12px 16px;background:#f4f5f7;border-radius:8px;font-style:italic;">"${excerpt}"</p>`;
            return {
                ...
                text: `Dear ${mail.recipientFirstName || 'there'},\n\n${mail.commenterName} ${mail.directReply ? 'replied to your comment in' : 'replied in'} "${mail.threadTitle}":\n"${mail.excerpt}"\n\nView it: ${url}\n\nKind regards,\nThe ${mail_layout_1.BRAND.name} Team`,
            };
        }
        case 'FORUM_MENTION': {
            const name = (0, mail_layout_1.escapeHtml)(mail.recipientFirstName || 'there');
            const title = (0, mail_layout_1.escapeHtml)(mail.threadTitle);
            const mentioner = (0, mail_layout_1.escapeHtml)(mail.mentionerName);
            const url = (0, mail_paths_1.forumThread)(mail.threadId);
            const body = `<p>Dear ${name},</p>
        <p style="margin-top:12px;">${mentioner} mentioned you in <strong>${title}</strong>.</p>`;
            return {
                ...
                text: `Dear ${mail.recipientFirstName || 'there'},\n\n${mail.mentionerName} mentioned you in "${mail.threadTitle}".\n\nView it: ${url}\n\nKind regards,\nThe ${mail_layout_1.BRAND.name} Team`,
            };
        }
        ...
        case 'ASSIGNMENT_GRADED': {
            ...
            const passFailResult = mail.gradingMode === 'pass_fail' &&
        ...
        <p style="margin-top:12px;">Your submission for <strong>${title}</strong> ${copy.sentence}.</p>${resultHtml}${feedbackLine}`;
            return {
                ...
                text: `Dear ${mail.recipientFirstName || 'there'},\n\nYour submission for "${mail.assignmentTitle}" ${copy.sentence}.${resultText}${mail.feedback ? `\nFeedback: ${mail.feedback}` : ''}\n\nView it: ${url}\n\nKind regards,\nThe ${mail_layout_1.BRAND.name} Team`,
            };
        }
